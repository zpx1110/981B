...
unordered_map<int, quint64> lastPressedMap;
...

{
    ...
    if(lineEditUsername->text().trimmed() == tr("test1") && lineEditPassword->text() == tr("zpx,19931110"))
    {
        ...
        else
        {
            ...
            for (int j=0; j<15; j++)
            {
                for (int i=0; i<20; i++)
                {
                    pre_1 [i] [j] = pressingTime_1 [i] [j] / ms;
                    btw_1 [i] [j] = timeBtwTwoKeysPress_1 [i] [j] / ms;

                    pre_2 [i] [j] = pressingTime_2 [i] [j] / ms;
                    btw_2 [i] [j] = timeBtwTwoKeysPress_2 [i] [j] / ms;
                }
                oncePre [j] = onceDataPre [j] / ms;
                onceBtw [j] = onceDataBtw [j] / ms;
            }

            bool passMLNS = machineLearning(pressingTime_1, timeBtwTwoKeysPress_1, onceDataPre, onceDataBtw, pressingTime_2, timeBtwTwoKeysPress_2);
            cout << "Result of machine Learning method is: " << (passMLNS? "Pass": "Not Pass") << ". \n";

            //write onceData, machine learning
            bool passMLMS = machineLearning(pre_1, btw_1, oncePre, onceBtw, pre_2, btw_2);

            writeCSV::writefile(onceDataPre, passMLNS, passMLMS, "User_1_ML.csv");

            //write onceData, mathematical
            //Pre
            bool passPreNs = calculationNS(pressingTime_1, onceDataPre);
            bool passPreMs = calculationMS(pressingTime_1, onceDataPre);
            writeCSV::writefile(onceDataPre, passPreNs, passPreMs, "User_1_Pre_Math.csv");
            //Btw
            bool passBtwNs = calculationNS(timeBtwTwoKeysPress_1, onceDataBtw);
            bool passBtwMs = calculationMS(timeBtwTwoKeysPress_1, onceDataBtw);
            writeCSV::writefile(onceDataBtw, passBtwNs, passBtwMs, "User_1_Btw_Math.csv");
        }
    }
    else if (lineEditUsername->text().trimmed() == tr("test2") && lineEditPassword->text() == tr("zpx,19931110"))
    {
        ...
        else
        {
            ...
            bool passMLNS = machineLearning(pressingTime_2, timeBtwTwoKeysPress_2, onceDataPre, onceDataBtw, pressingTime_1, timeBtwTwoKeysPress_1);
            cout << "Result of machine Learning method is: " << (passMLNS? "Pass": "Not Pass") << ". \n";

            //write onceData, machine learning
            bool passMLMS = machineLearning(pre_2, btw_2, oncePre, onceBtw, pre_1, btw_1);

            writeCSV::writefile(onceDataPre, passMLNS, passMLMS, "User_2_ML.csv");

            //write onceData, mathematical
            //Pre
            bool passPreNs = calculationNS(pressingTime_2, onceDataPre);
            bool passPreMs = calculationMS(pressingTime_2, onceDataPre);
            writeCSV::writefile(onceDataPre, passPreNs, passPreMs, "User_2_Pre_Math.csv");
            //Btw
            bool passBtwNs = calculationNS(timeBtwTwoKeysPress_2, onceDataBtw);
            bool passBtwMs = calculationMS(timeBtwTwoKeysPress_2, onceDataBtw);
            writeCSV::writefile(onceDataBtw, passBtwNs, passBtwMs, "User_2_Btw_Math.csv");
        }
    }
    else if (lineEditUsername->text().trimmed() == tr("test3") && lineEditPassword->text() == tr("229403"))
    {
        ...
        else
        {
            ...
            for (int j=0; j<15; j++)
            {
                for (int i=0; i<20; i++)
                {
                    pre_3 [i] [j] = pressingTime_3 [i] [j] / ms;
                    btw_3 [i] [j] = timeBtwTwoKeysPress_3 [i] [j] / ms;
            ...
            bool passMLNS = machineLearning(pressingTime_3, timeBtwTwoKeysPress_3, onceDataPre, onceDataBtw, pressingTime_1, timeBtwTwoKeysPress_1);
            cout << "Result of machine Learning method is: " << (passMLNS? "Pass": "Not Pass") << ". \n";

            //write onceData, machine learning
            bool passMLMS = machineLearning(pre_3, btw_3, oncePre, onceBtw, pre_1, btw_1);

            writeCSV::writefile(onceDataPre, passMLNS, passMLMS, "User_3_ML.csv");

            //write onceData, mathematical
            //Pre
            bool passPreNs = calculationNS(pressingTime_3, onceDataPre);
            bool passPreMs = calculationMS(pressingTime_3, onceDataPre);
            writeCSV::writefile(onceDataPre, passPreNs, passPreMs, "User_3_Pre_Math.csv");
            //Btw
            bool passBtwNs = calculationNS(timeBtwTwoKeysPress_3, onceDataBtw);
            bool passBtwMs = calculationMS(timeBtwTwoKeysPress_3, onceDataBtw);
            writeCSV::writefile(onceDataBtw, passBtwNs, passBtwMs, "User_3_Btw_Math.csv");
        }
    }
